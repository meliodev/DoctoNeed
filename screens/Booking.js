...
onAppointmentClick() {
  this.props.navigation.navigate('Symptomes', {doctor: this.doctor, fullDate: this.state.fullDate,
    ...
    return (
      ...
      return (
        ...
        <TouchableHighlight key={index} style={dayStyle}
                            onPress={() => this.setState({ daySelected: dayMonth[index].day, 
                                                           monthSelected: dayMonth[index].month, 
                                                           yearSelected: dayMonth[index].year,
                                                           fullDate: dayMonth[index].fullDate,
                                                           day: index,
                                                            })}
                            style={ index === this.state.day ? { backgroundColor: this.state.backgroundColor, 
                                                                  paddingTop: 10,
                                                                 paddingBottom: 12,
                                                                 borderRadius: 5,
                                                                 height: SCREEN_WIDTH*0.095,
                                                                 width: SCREEN_WIDTH*0.095 } : dayStyle}>
        ...
        (doctorSchedule[index].map( (timeslot, index) => {
          ...
          return (
            <TouchableHighlight key={index} style={timeStyle}
                                onPress={() => this.setState({ timeSelected: doctorSchedule[day][index].time,
                                                               time: index })}

                                style={ index === this.state.time ? { backgroundColor: this.state.backgroundColor, 
                                                                margin: 5, paddingTop: 12,
                                                                paddingBottom: 12,
                                                                borderRadius: 5,
                                                                height: SCREEN_WIDTH*0.095,
                                                                width: SCREEN_WIDTH*0.19 } : timeStyle}>
    ...
    return (
        <View style={styles.container}>
          ...
          <View style={styles.days}>
              <Swiper style={styles.wrapper} showsPagination={false} loop={false} nextButton= {<Text style={{marginTop: 200}}>›</Text>} prevButton= {<Text style={{marginTop: 200}}>‹</Text>} buttonWrapperStyle={{backgroundColor: 'transparent',
                      flexDirection: 'row', position: 'absolute', top: 0, left: 0, flex: 1, paddingHorizontal: 10, paddingVertical: 0,
                      justifyContent: 'space-between', alignItems: 'center'}} index={global.pos}>
                {this.swiperView()}
              </Swiper>
          </View>
