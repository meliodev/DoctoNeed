...
import firebase, { notifications } from '../../configs/firebase'
...
// import PreviousAppointments from '../User/PreviousAppointments'
// import NextAppointments from '../User/NextAppointments'
...
class Home2 extends React.Component {
  constructor(props) {
    console.log('CONSTRUCTING HOME2')
    super(props);
  ...
  async componentDidMount() {
    ...
    if (firebase.auth().currentUser) {
      console.log('HOME 2 SCREEN COMPONENT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
...
// else {
//   //Check if user on the current device has changed his ROLE
//   //Used another user id
//   REFS.fcmtokens.where('fcmToken', '==', fcmToken).get().then((querySnapshot) => {
//     querySnapshot.forEach(doc => {
//       if (doc.exists && this.props.role !== doc.data().role) {
//         //delete token from the old user role
//         REFS.fcmtokens.doc(doc.id).delete()
//         //add token to the current user role
//         let tok = {
...
//           createdAt: moment(new Date()).format()
//         }
//         REFS.fcmtokens.doc(firebase.auth().currentUser.uid).set(tok)
//           .then(() => console.log('fcm token added to admins devices list'))
//           .catch((err) => console.error(err))
//       }
//     })
